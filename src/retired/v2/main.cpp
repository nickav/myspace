...
void WriteBlogListItem(Arena *arena, Post *post, String post_link)
{
    ...
    auto size = ParseImageInfo(image->data);
    Write(arena, "<a href='%S'><div><img src='%S' width='%d' height='%d' />%S</div></a>", post_link, image_path, size.width, size.height, post->title);
}
...
int main() {
    ...
    // Config
    config = {};
    config.asset_dir  = path_join(project_root, S("data"));
